...
Nous étudions des données $t_1,\ldots, t_n$ censurés à droite (censure non informative)
$$
...
\begin{ex}
	\begin{enumerate}
		...
		\item Pour une variable explicative catégorielle $Z\in E$, on crée autant de variables $\mathbb{I}_{Z = z}$ que de modalité $z\in E$. Chacune de ces modalités est alors associé à un coefficient $\beta_z$ de telle sorte que 
		$$
		h(t) = h_0(t)\e^{\beta_z},\text{ lorsque }Z =z.
		$$
...
La vraisemblance du modèle pour les données $\Data = (x_i,\delta_i, z_i)_{i=1,\ldots, n}$ s'écrit 
\begin{equation}\label{eq:likelihood_cox}
\mathcal{L}(\Data;\theta) =\prod_{i=1}^n \left[h_0(x_i)\e^{\,^tz_i\cdot \beta}\right]^{\delta_i}\cdot\exp\left(-\e^{\,^tz_i\cdot\beta} \int_0^{x_i}h_0(s)\text{d}s\right) ,
\end{equation}
...
\begin{equation*}
\mathcal{L}(\Data;\theta) =\prod_{i=1}^n \prod_{k=1}^K\left[\alpha_k\e^{\,^tz_i\cdot \beta}\right]^{\delta_i\mathbb{I}_{x_i = v_k}}\cdot\exp\left(-\e^{\,^tz_i\cdot\beta}\alpha_k \int_0^{x_i}\ind_{[v_{k}, v_{k+1})}(s)\text{d}s\right) ,
\end{equation*}
On passe au log
\begin{eqnarray*}
l(\Data;\theta)
&=& \sum_{i=1}^n\sum_{k=1}^K\left[\log(\alpha_k)+ \,^tz_i\beta\right]\delta_i\ind_{x_i = v_k} -\e^{z_i\beta}\sum_{k=1}^K\alpha_k\int_0^{x_i}\ind_{[v_{k}, v_{k+1})}(s)\text{d}s\\
&=& \sum_{k=1}^K\log(\alpha_k)d(v_k)+s_k\beta -\alpha_k\sum_{i=1}^{n}\int_0^{x_i}\e^{\,^tz_i\beta}\ind_{[v_{k}, v_{k+1})}(s)\text{d}s\\
&=& \sum_{k=1}^K\log(\alpha_k)d(v_k)+s_k\beta -\alpha_k\left\{\sum_{i=1}^{n}\e^{\,^tz_i\beta}\left[(v_{k+1} - v_{k})\ind_{x_i\geq v_{k+1}} +(x_i - v_{k})\ind_{[v_{k}, v_{k+1})}(x_i)\right]\right\}\\
...
où 
\begin{eqnarray*}
E_k(\beta) &=& \sum_{i=1}^{n}\e^{\,^tz_i\beta}\left[(v_{k+1} - v_{k})\ind_{x_i\geq v_{k+1}} +(x_i - v_{k})\ind_{[v_{k}, v_{k+1})}(x_i)\right],\\
&=& \sum_{i=1}^{n}\e^{\,^tz_i\beta}\left[v_{k+1}\land x_i - v_{k}\right]_+.
\end{eqnarray*}
est l'exposition.
% \begin{remark}
